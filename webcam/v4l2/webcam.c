...
{
    int i = 0;
    fprintf(stderr, "[v4l2] A segmentation fault occured. Cleaning up...\n");
...
struct webcam *webcam_open(const char *dev)
{
    ...
    if (!S_ISCHR(st.st_mode)) {
        fprintf(stderr, "[v4l2] %s is not a char device\n", dev);
        return NULL;
    }
    ...
    if (-1 == _ioctl(fd, VIDIOC_QUERYCAP, &cap)) {
        if (EINVAL == errno) {
            fprintf(stderr, "[v4l2] %s is no V4L2 device\n", dev);
            return NULL;
        } else {
            fprintf(stderr, "[v4l2] %s: could not fetch video capabilities\n", dev);
            return NULL;
        }
    }
...
int webcam_resize(webcam_t *w, uint16_t width, uint16_t height)
{
    ...
    fprintf(stderr, "[v4l2] %s: set image format to %ux%u using %s\n", w->name, w->width, w->height, pixelformat);
    ...
    fprintf(stderr, "[v4l2] Preparing %d buffers for %s\n", req.count, w->name);
